...
- The sealed manifest's `extension_hash` = a reproducible SHA-256 **tree-hash** over the plugin distribution's extracted file tree (`DirectoryHash.sha256`: files sorted by relative path, digesting `<relative-path-utf8>\0<file-bytes>` per file) — **not** a hash of the `.zip` archive bytes, since zip byte layout isn't reproducible across tools. Computed at build/CI time by the `computeExtensionHash` Gradle task (over the extracted distribution) and at seal time (over the plugin's installed directory). Both go through the one shared `DirectoryHash.sha256`, but that alone does **not** make them agree — the digest covers each file's path *relative to the root it was given*, so the two callers must also pass the same tree level. They once did not (the build passed the extraction staging dir, which contains the `recorder/` plugin dir, while seal passes the installed plugin dir itself), and every build-time hash was a value no installed plugin could reproduce. The build side must resolve the distribution root inside its staging dir (`pluginDistributionRoot`), which fails loudly on an unexpected layout. That hash must be added to the monorepo allowlist or every submission gets flagged.
